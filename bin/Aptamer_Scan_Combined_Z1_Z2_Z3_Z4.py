...
def function_aptamer_scan_meta_Z_score(w_motif_Z_score, aptamer_seq):
    # Python 3では、dict.keys()はビューとなるため、リスト化して先頭要素を取得
    ...
    for i in range(len(aptamer_seq) - motif_len + 1):
        ...
        # 辞書に存在しないキーの場合はKeyErrorとなるので、必要に応じてエラーチェックを追加
...
if Sort_logical.upper() == 'FALSE':
    with open(Aptamer_file, 'r') as infile, open(Output_file, 'w') as outfile:
        header = ('Aptamer.Seq' + '\t' +
                  'Z1.Scan' + '\t' + 'Z1.MetaScore' + '\t' +
                  'Z2.Scan' + '\t' + 'Z2.MetaScore' + '\t' +
        ...
        for line in infile:
            ...
            outfile.write(function_aptamer_scan_meta_Z_score(w_motif_Z2, Seq) + '\t')
    ...
    with open(Aptamer_file, 'r') as infile:
        for line in infile:
            ...
            this_Z2 = function_aptamer_scan_meta_Z_score(w_motif_Z2, Seq)
            ...
            this_score = this_combined.split()[-1]
            out_list.append([float(this_score),
                             Seq + '\t' + this_Z1 + '\t' + this_Z2 + '\t' +
                             this_Z3 + '\t' + this_Z4 + '\t' + this_combined + '\n'])
    # 降順ソート（combined Zスコアでソート）
